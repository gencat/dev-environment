...
su - canigo -c "cd $TEMPO_DIR; $MVN_DIR/bin/mvn -B archetype:generate -DarchetypeGroupId=cat.gencat.ctti -DarchetypeArtifactId=plugin-canigo-archetype-rest -DarchetypeVersion=1.6.2 -DartifactId=AppCanigo162 -DgroupId=cat.gencat.ctti -Dversion=1.0"
su - canigo -c "cd $TEMPO_DIR/AppCanigo; $MVN_DIR/bin/mvn -B clean package failsafe:integration-test"
